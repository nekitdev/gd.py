...
class Object(Model):
    ...
    #     index=71,
    #     aliases=("follow_group_id", "target_pos_id", "center_id", "secondary_id"),
    # )
    ...
    # target_position_coordinates: TargetPositionCoordinates = EnumField(
    #     index=101, enum_type=TargetPositionCoordinates, from_field=IntField
    # )
...
class ColorChannel(Model):
    ...
    player_color: PlayerColor = EnumField(
        index=4, enum_type=PlayerColor, from_field=IntField, default=PlayerColor.NOT_USED
    )
...
DEFAULT_COLORS = (
    ...
    ColorChannel("G1").set_color(0x0066FF),
...
CC = TypeVar("CC", bound="ColorCollection")


class ColorCollection(Set[ColorChannel]):
    def __init__(
        self,
        iterable: Optional[Iterable[ColorChannel]] = None,
    ...
    @classmethod
    def new(cls: Type[CC], *args: ColorChannel, use_default: bool = True) -> CC:
    ...
    def clone(self: CC) -> CC:
        return self.__class__(channel.clone() for channel in self)

    def difference(self: CC, other: Iterable[ColorChannel]) -> CC:
        return self.__class__(super().difference(other))

    def intersection(self: CC, other: Iterable[ColorChannel]) -> CC:
        return self.__class__(super().intersection(other))

    def symmetric_difference(self: CC, other: Iterable[ColorChannel]) -> CC:
        return self.__class__(super().symmetric_difference(other))

    def union(self: CC, other: Iterable[ColorChannel]) -> CC:
        return self.__class__(super().union(other))

    def update(self, other: Iterable[ColorChannel]) -> CC:
        super().update(other)
    ...
    def get(self, directive_or_id: Union[int, str]) -> Optional[ColorChannel]:
        ...
        return iter(self).get_or_none(id=id)

    def __or__(self: CC, other: Set[ColorChannel]) -> CC:
        return self.__class__(super().__or__(other))

    def __xor__(self: CC, other: Set[ColorChannel]) -> CC:
        return self.__class__(super().__xor__(other))

    def __sub__(self: CC, other: Set[ColorChannel]) -> CC:
        return self.__class__(super().__sub__(other))

    def __and__(self: CC, other: Set[ColorChannel]) -> CC:
        return self.__class__(super().__and__(other))

    def __ror__(self: CC, other: Set[ColorChannel]) -> CC:
        return self.__class__(super().__ror__(other))

    def __rxor__(self: CC, other: Set[ColorChannel]) -> CC:
        return self.__class__(super().__rxor__(other))

    def __rsub__(self: CC, other: Set[ColorChannel]) -> CC:
        return self.__class__(super().__rsub__(other))

    def __rand__(self: CC, other: Set[ColorChannel]) -> CC:
...
class Header(Model):
    ...
    platformer_mode: bool = BoolField(index="kA22", default=False)
...
class LevelAPI(Model):
    ...
    length: LevelLength = BaseField(
        index="k23",
        de=partial(enum_from_value, enum_type=LevelLength),
        ser=enum_to_value,
    )
    ...
    binary_version: Version = BaseField(
        index="k50",
        de=Version.from_number,
        ser=Version.to_number,
        default=Version(3, 5),
    )
