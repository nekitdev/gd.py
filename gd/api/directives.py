...
def get_id(string: str, into_enum: bool = False, delim: str = delimiter) -> Any:
...
supported = {
    name: {
        ...
        for member_name, member in enum.__members__.items()  # type: ignore
    }
...
supported.get("speed", {}).update(
    {
        f"x{simple}": f"speed:{speed.name.casefold()}"  # type: ignore
        for simple, speed in speeds.items()
    }
)


def get_dir(string: str, begin: str, delim: str = delimiter) -> str:
